...
Во-первых, к глобальным переменным добавилась трехмерная матрица нормалей: `var nMatrix = mat3.create();`. Это один из объектов, который 
затем передается в шейдер для установки освещения.
...
Здесь мы, во-первых, устанавливаем позицию источника света: в данном случае это точка с координатами (0.0, 10.0, 5.0). А также устанавливаем 
rgb-значения для трех отражений света.

В функции `setMatrixUniforms()` в конце добавляем установку матрицы нормалей. Саму эту матрицу мы получаем в функции 
`setupWebGL()` с помощью специального метода библиотеки glMatrix: `mat3.normalFromMat4(nMatrix, mvMatrix);`

И в функции `initBuffers()` создаем буфер нормалей. Так как в данном случае объект по сути представляет куб, то тут очень просто определить нормали. 
...
![Создание освещения в WebGL](https://metanit.com/web/webgl/pics/8.5.png)
